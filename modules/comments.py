		...
		for reply in token_comment.replies:
			if reply.author:
				...
				if str(reply.author.name).lower() == data["running_username"].lower():
					reply.edit(data["msg_confirmation"] % (CSawardee,CSawardee,data["running_subreddit"],awardee,data["running_username"],data["running_subreddit"],data["running_username"])).distinguish()
					edited_reply = True
		if edited_reply == False:
			logging.debug("Leaving new comment")
			token_comment.reply(data["msg_confirmation"] % (CSawardee,CSawardee,data["running_subreddit"],awardee,data["running_username"],data["running_subreddit"],data["running_username"])).distinguish()
		logging.info("Confirmation Message Sent")
		wiki.start(data,r,token_comment,token_comment.author.name,CSawardee,flair_count)
